...
import org.ladlb.directassemblee.synthesis.R
...
class RatesFragment : AbstractFragment() {
    ...
    private val presenter: RateGetPresenter by inject()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        presenter.viewState.collect(this) {
            when (it) {
                is Result -> onActivityRatesReceived(it.rates)
                is Error -> onGetActivityRatesRequestFailed()
            }
        }
    }

    private fun onActivityRatesReceived(rates: List<Rate>) {
        recyclerView.adapter = RateAdapter(rates.toCollection(ArrayList()))
        loadingView.visibility = GONE
    }

    private fun onGetActivityRatesRequestFailed() {
